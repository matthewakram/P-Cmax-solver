...
impl BranchAndBoundLogging {
    fn solve_rec(
        ...
        if fur_job != usize::MAX {
            ...
            let sol = self.solve_rec(instance, part_sol, ret, lower, best_makespan_found, timeout, log_file);
            if sol.is_err() {
                return Err(());
            }
            ...
            if sol.is_some() {
                ...
                let better_sol =
                    self.solve_rec(instance, part_sol, ret, lower, best_makespan_found, timeout, log_file);
...
impl SolverManager for BranchAndBoundLogging {
    ...
    fn solve(
        ...
        let mut file: File = File::create("log.log").unwrap();
        let sol = self.solve_rec(instance, &mut part_sol, ret, lower, upper.makespan, timeout, &mut file);
        if sol.is_err() {
            return None;
        }
        let sol = sol.unwrap();
