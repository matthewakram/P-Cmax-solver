...
mod tests {
    use crate::{
        ...
        encoding::{
            binary_arithmetic,
            cardinality_networks::{basic_merge, basic_sort, half_merge, half_sort},
            encoder::{Clause, Clauses, VarNameGenerator},
        },
        input_output::to_dimacs,
    ...
    pub fn binary_arithmetic_sum_test() {
        ...
        let number1: binary_arithmetic::BinaryNumber =
            binary_arithmetic::BinaryNumber::new(8, &mut name_generator);
        let number2: binary_arithmetic::BinaryNumber =
            binary_arithmetic::BinaryNumber::new(7, &mut name_generator);
        ...
        let (sum, sum_assertion) =
            binary_arithmetic::bounded_sum_encoding(&number1, &number2, 5, &mut name_generator);
    ...
    pub fn simp_sorter_test() {
        ...
        clauses.add_clause(Clause {
            vars: vec![(vars[0] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![(vars[1] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![(vars[2] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![(vars[3] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![(vars[4] as i32)],
        });
    ...
    pub fn merge_test() {
        ...
        clauses.add_clause(Clause {
            vars: vec![-(vars1[0] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![-(vars1[1] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![-(vars1[2] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![-(vars1[3] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![-(vars1[4] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![(vars2[0] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![-(vars2[1] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![-(vars2[2] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![-(vars2[3] as i32)],
        });
        clauses.add_clause(Clause {
            vars: vec![-(vars2[4] as i32)],
        });
