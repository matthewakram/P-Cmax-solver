...
use crate::{problem_instance::{problem_instance::ProblemInstance, solution::Solution}, bounds::{bound::Bound, upper_bounds::{lpt, lptp, lptpp, mss}}, encoding::{encoder::Encoder, basic_with_precedence::Precedence, pb_bdd_inter_better::PbInterDyn}, solvers::sat_solver::{sat_solver_manager, kissat::Kissat}, makespan_scheduling::linear_makespan::LinearMakespan, common::timeout::Timeout};
...
impl Lifting {
    ...
    pub fn new() -> Lifting {
        return Lifting{seed: rand::thread_rng().gen()}
    }

    fn bound_instance(&self, instance: &ProblemInstance, lower_bound: usize, remaining_time: &Timeout) -> (usize, Solution){
...
impl Bound for Lifting{
    fn bound(&self, problem: &ProblemInstance, lower_bound: usize, upper_bound: Option<crate::problem_instance::solution::Solution>, timeout: &Timeout) -> (usize, Option<crate::problem_instance::solution::Solution>) {
        ...
        for instance in &instances_to_bound {
            ...
            let instance_bound_time = bounding_timeout.clone_with_new_timeout(bounding_timeout.remaining_time() / num_instances_remaining);
