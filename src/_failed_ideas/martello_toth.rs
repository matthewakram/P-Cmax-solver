use crate::{
    bounds::bound::Bound,
    problem_instance::{self, problem_instance::ProblemInstance},
};
...
pub fn get_j1(instance: &ProblemInstance, makespan_to_test: usize, p: usize) -> Vec<usize> {
    return instance
        .job_sizes
        .iter()
        .filter(|pj| **pj <= makespan_to_test / 2 && makespan_to_test - p < **pj)
        .map(|x| *x)
        .collect();
}
...
pub fn get_j2(instance: &ProblemInstance, makespan_to_test: usize, p: usize) -> Vec<usize> {
    return instance
        .job_sizes
        .iter()
        .filter(|pj| makespan_to_test / 2 < **pj && **pj <= makespan_to_test - p)
        .map(|x| *x)
        .collect();
}
...
pub fn get_j3(instance: &ProblemInstance, makespan_to_test: usize, p: usize) -> Vec<usize> {
    return instance
        .job_sizes
        .iter()
        .filter(|pj| p <= **pj && **pj <= makespan_to_test / 2)
        .map(|x| *x)
        .collect();
}

pub fn inner_sum(j2: Vec<usize>, j3: Vec<usize>, c: usize) -> usize {
...
pub fn bin_packing_bound(instance: &ProblemInstance, makespan_to_test: usize) -> usize {
    ...
    for i in 0..instance.num_jobs {
        ...
        let bound = j1.len() + j2.len() + inner_sum(j2, j3, makespan_to_test);
...
impl Bound for MartelloToth {
    fn bound(
        ...
        lower_bound: usize,
        upper_bound: Option<problem_instance::solution::Solution>,
        _timeout: f64,
    ) -> (usize, Option<problem_instance::solution::Solution>) {
