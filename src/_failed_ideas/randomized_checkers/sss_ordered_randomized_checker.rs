use crate::{
    ...
    encoding::{encoder::Encoder, pb_bdd_pysat::PbPysatEncoder},
...
impl SSSOrderedRandomizedChecker {
    fn fill_proc(
        ...
        for job in &self.job_order {
            let job = *job;
            if part.possible_allocations[job].len() != 1
                && part.possible_allocations[job].contains(&proc)
            {
        ...
        for job in 0..out.instance.num_jobs {
            ...
            let index_of_currently_filled_proc = out.possible_allocations[job]
                .iter()
                .enumerate()
                .find(|(_, x)| **x == proc);
