...
impl Encoder for BasicWithFillUp {
    fn basic_encode(
        ...
        for job in 0..partial_solution.instance.num_jobs {
            let mut already_assigned_vars_i: Vec<Option<usize>> =
                vec![self.basic.problem.position_vars[job][0]];
            let mut previous_already_assigned_var: Option<usize> =
                self.basic.problem.position_vars[job][0];
            for processor in 1..partial_solution.instance.num_processors {
                ...
                if self.basic.problem.position_vars[job][processor].is_none() {
                    ...
                    let next_already_assigned_var: Option<usize> =
                        Some(self.basic.problem.var_name_generator.next());
                    if next_already_assigned_var.is_some() {
                        ...
                        if previous_already_assigned_var.is_some() {
                            ...
                            clauses.push(Clause {
                                vars: vec![
                                    -(previous_already_assigned_var.unwrap() as i32),
                                    -(self.basic.problem.position_vars[job][processor].unwrap()
                                        as i32),
                                ],
                            });
                        }
        ...
        for job in 0..partial_solution.instance.num_jobs {
            for processor in 0..partial_solution.instance.num_processors {
                if already_assigned_vars[job][processor].is_some()
                    && self.basic.problem.position_vars[job][processor].is_some()
                {
                    ...
                    if goal_sum
                        > self.basic.partial_sum_variables[processor][job]
                            .as_ref()
                            .unwrap()
                            .max
                    {
                    ...
                    let mut ne_clause = binary_arithmetic::not_equals_constant_encoding(
                        self.basic.partial_sum_variables[processor][job]
                            .as_ref()
                            .unwrap(),
                        makespan - partial_solution.instance.job_sizes[job],
                    );
                    ne_clause
                        .vars
                        .push(self.has_already_been_assigned_vars[job][processor].unwrap() as i32);
