use super::precedence_relation_generator::{PrecedenceRelation, PrecedenceRelationGenerator};
        ...
        for job1 in 0..instance.num_jobs - 2 {
            for job2 in (job1 + 1)..instance.num_jobs - 1 {
                ...
                for job3 in (job2 + 1)..instance.num_jobs {
                    if instance.job_sizes[job3] + instance.job_sizes[job2]
                        == instance.job_sizes[job1]
                    {
                        total_num += 1;
                        out.push(PrecedenceRelation {
                            comes_first: job1,
                            comes_second: vec![job2, job3],
                        })
                    }
                }
            }
